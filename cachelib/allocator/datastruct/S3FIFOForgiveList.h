...
class S3FIFOForgiveList {
 ...
 private:
  ...
  bool shouldForgive(const T& node) noexcept {
    ...
    if (embMgr_->get_access_count(id) < minAccessCount_) return false;
    if (!embMgr_->has_embedding(id)) return false;
    const double sim = embMgr_->avg_top_k_similarity_to_recent(id, topK_);
